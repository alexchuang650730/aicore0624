...
__all__ = [
    ...
    # Manus Adapter API端點處理方法 (新增)
    async def handle_manus_request(self, endpoint: str, request_data: Dict) -> Dict:
        """處理 Manus 相關請求"""
        try:
            if self.manus_adapter is None:
                return {
                    "success": False,
                    "error": "Manus Adapter 未初始化",
                    "timestamp": datetime.now().isoformat()
                }
            
            # 根據端點路由到對應的處理方法
            if endpoint == "/api/manus/requirement/analyze":
                return await self._handle_manus_requirement_analysis(request_data)
            
            elif endpoint == "/api/manus/task/analyze":
                return await self._handle_manus_task_analysis(request_data)
            
            elif endpoint == "/api/manus/ui/review":
                return await self._handle_manus_ui_review(request_data)
            
            elif endpoint == "/api/manus/cross-task/analyze":
                return await self._handle_manus_cross_task_analysis(request_data)
            
            elif endpoint == "/api/manus/status":
                return await self.manus_adapter.get_manus_status()
            
            else:
                return {
                    "success": False,
                    "error": f"未知的 Manus 端點: {endpoint}",
                    "timestamp": datetime.now().isoformat()
                }
    ...
    async def _handle_manus_requirement_analysis(self, request_data: Dict) -> Dict:
        """處理 Manus 需求分析請求"""
        try:
            ...
            if not requirement_text or not target_entity:
                return {
                    "success": False,
                    "error": "缺少必要參數: requirement_text 或 target_entity"
                }
            
            # 調用 Manus Adapter 進行需求分析
            result = await self.manus_adapter.analyze_requirement(
                requirement_text, target_entity, context
            )
            ...
            return {
                ...
                "data": {
                    ...
                    "cross_task_relations": result.cross_task_relations,
    ...
    async def _handle_manus_task_analysis(self, request_data: Dict) -> Dict:
        """處理 Manus 任務分析請求"""
        try:
            from components.manus_adapter_mcp import ManusRequest, ManusRequestType, ManusAnalysisScope
            
            task_id = request_data.get("task_id", "")
            analysis_scope = request_data.get("analysis_scope", "single_task")
            context = request_data.get("context", {})
            
            request = ManusRequest(
                request_id=f"task_{int(time.time())}",
                request_type=ManusRequestType.TASK_ANALYSIS,
                content=f"分析任務 {task_id}",
                target_entity=task_id,
                analysis_scope=ManusAnalysisScope(analysis_scope),
                context=context
            )
            
            result = await self.manus_adapter.process_manus_request(request)
            ...
            logger.error(f"❌ Manus 任務分析失敗: {e}")
            return {
                "success": False,
                "error": str(e)
            }
    
    async def _handle_manus_ui_review(self, request_data: Dict) -> Dict:
        """處理 Manus UI 設計審查請求"""
        try:
            from components.manus_adapter_mcp import ManusRequest, ManusRequestType, ManusAnalysisScope
            ...
            request = ManusRequest(
                ...
                request_type=ManusRequestType.UI_DESIGN_REVIEW,
                content=f"審查 UI 組件: {ui_component}",
                target_entity=ui_component,
                analysis_scope=ManusAnalysisScope.SINGLE_TASK,
                context={**context, "design_requirements": design_requirements}
            )
            ...
            return {
                ...
                "data": {
                    "request_id": result.request_id,
                    "ui_component": result.target_entity,
                    "expert_insights": result.expert_insights,
            ...
            logger.error(f"❌ Manus UI 審查失敗: {e}")
            return {
                "success": False,
                "error": str(e)
            }
    
    async def _handle_manus_cross_task_analysis(self, request_data: Dict) -> Dict:
        """處理 Manus 跨任務分析請求"""
        try:
            from components.manus_adapter_mcp import ManusRequest, ManusRequestType, ManusAnalysisScope
            ...
            request = ManusRequest(
                request_id=f"cross_{int(time.time())}",
                request_type=ManusRequestType.CROSS_TASK_ANALYSIS,
                content=f"跨任務分析: {', '.join(task_list)}",
                target_entity=f"CROSS_ANALYSIS_{len(task_list)}_TASKS",
                analysis_scope=ManusAnalysisScope.MULTI_TASK,
                context={**context, "task_list": task_list, "analysis_focus": analysis_focus}
            )
            ...
            return {
                ...
                "data": {
                    ...
                    "cross_task_relations": result.cross_task_relations,
                    "expert_insights": result.expert_insights,
                    "coordination_recommendations": result.recommendations,
                    "confidence_score": result.confidence_score,
                    "processing_time": result.processing_time
                },
                "timestamp": result.timestamp.isoformat()
            ...
            logger.error(f"❌ Manus 跨任務分析失敗: {e}")
            return {
                "success": False,
                "error": str(e)
            }
    
    async def process_manus_requirement(self, requirement_text: str, target_entity: str = "REQ_001", context: Dict = None) -> Dict:
        """便捷方法：處理 Manus 需求 - 對外 API"""
        logger.info(f"🎯 處理 Manus 需求: {target_entity}")
        
        try:
            result = await self.manus_adapter.analyze_requirement(
                requirement_text, target_entity, context or {}
            )
            ...
            return {
                ...
                "analysis_result": {
                    "target_entity": result.target_entity,
                    "requirements_list": result.recommendations[:5],  # 前5個作為需求列表
                    "manus_actions": result.recommendations[5:10],    # 接下來5個作為行動項目
                    "file_references": result.file_references,
                    "cross_task_analysis": result.cross_task_relations,
                    "expert_insights": result.expert_insights,
                    "confidence_score": result.confidence_score,
                    "processing_time": result.processing_time
                },
