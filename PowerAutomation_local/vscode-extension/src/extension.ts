...
let smartUIController: SmartUIController;
...
export async function activate(context: vscode.ExtensionContext) {
    ...
    smartUIController = new SmartUIController(context);
...
function registerSmartUICommands(context: vscode.ExtensionContext) {
    // 角色切換命令
    context.subscriptions.push(
        vscode.commands.registerCommand('powerautomation.smartui.switchToAdmin', async () => {
            await switchRole(UserRole.ADMIN);
        })
    );
    
    context.subscriptions.push(
        vscode.commands.registerCommand('powerautomation.smartui.switchToDeveloper', async () => {
            await switchRole(UserRole.DEVELOPER);
        })
    );
    
    context.subscriptions.push(
        vscode.commands.registerCommand('powerautomation.smartui.switchToUser', async () => {
            await switchRole(UserRole.USER);
        })
    );
    
    // Claude 分析命令
    context.subscriptions.push(
        vscode.commands.registerCommand('powerautomation.smartui.analyzeSelection', async () => {
            await analyzeSelectedText('analysis');
        })
    );
    
    context.subscriptions.push(
        vscode.commands.registerCommand('powerautomation.smartui.reviewCode', async () => {
            await analyzeSelectedText('review');
        })
    );
    
    context.subscriptions.push(
        vscode.commands.registerCommand('powerautomation.smartui.explainCode', async () => {
            await analyzeSelectedText('explanation');
        })
    );
    
    context.subscriptions.push(
        vscode.commands.registerCommand('powerautomation.smartui.generateCode', async () => {
    ...
    // 系統狀態命令
    context.subscriptions.push(
        vscode.commands.registerCommand('powerautomation.smartui.showStatus', async () => {
            await showSystemStatus();
        })
    );
    
    context.subscriptions.push(
        vscode.commands.registerCommand('powerautomation.smartui.showRecommendations', async () => {
            await showRecommendations();
        })
    );
    
    context.subscriptions.push(
        vscode.commands.registerCommand('powerautomation.smartui.showUserAnalysis', async () => {
...
async function analyzeSelectedText(type: string): Promise<void> {
    if (!smartUIController) {
        vscode.window.showErrorMessage('SmartUI 系統未初始化');
        return;
    }
...
async function generateCodeFromRequirements(): Promise<void> {
    if (!smartUIController) {
        vscode.window.showErrorMessage('SmartUI 系統未初始化');
        return;
    }
...
async function showUserAnalysis(): Promise<void> {
    if (!smartUIController) {
        vscode.window.showErrorMessage('SmartUI 系統未初始化');
        return;
    }
...
async function showAnalysisResult(result: any): Promise<void> {
    // 創建新的文檔顯示分析結果
...
function getHealthStatusIcon(health: string): string {
    const icons = {
        'healthy': '✅',
        'warning': '⚠️',
        'error': '❌'
