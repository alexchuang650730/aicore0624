...
export class SmartUIClaudeService implements ClaudeService {
    private isInitialized = false;
    private apiKeys: Map<UserRole, string> = new Map();
    private requestCache: Map<string, { response: ClaudeResponse; timestamp: number }> = new Map();
    private readonly CACHE_TTL = 24 * 60 * 60 * 1000; // 24小時緩存
    private readonly API_BASE_URL = 'https://api.anthropic.com/v1/messages';
    ...
    // 角色特定的分析方法
    async analyzeRequirements(text: string, role: UserRole): Promise<any> {
    ...
    async generateCode(requirements: string, role: UserRole): Promise<any> {
        const request: ClaudeRequest = {
            id: this.generateRequestId(),
            type: 'generation',
    ...
    async reviewCode(code: string, role: UserRole): Promise<any> {
        const request: ClaudeRequest = {
            id: this.generateRequestId(),
            type: 'review',
            content: code,
    ...
    async explainCode(code: string, role: UserRole): Promise<any> {
        const request: ClaudeRequest = {
            id: this.generateRequestId(),
            type: 'explanation',
            content: code,
    ...
    private buildMessages(request: ClaudeRequest): any[] {
    ...
    private getSystemPrompt(role: UserRole, type: string): string {
    ...
    private getUserPrompt(request: ClaudeRequest): string {
    ...
    private async callClaudeAPI(apiKey: string, messages: any[], request: ClaudeRequest): Promise<any> {
    ...
    private generateMockResponse(request: ClaudeRequest): string {
        const responses = {
...
\`\`\``
    ...
    private processClaudeResponse(claudeResponse: any, request: ClaudeRequest): any {
    ...
    private extractInsights(content: string): string[] {
        // 簡單的洞察提取邏輯
        const insights = [];
        if (content.includes('功能需求')) insights.push('功能需求分析完成');
        if (content.includes('技術可行性')) insights.push('技術可行性評估');
        if (content.includes('業務價值')) insights.push('業務價值分析');
        return insights;
    }
    ...
    private extractRecommendations(content: string): string[] {
        ...
        if (content.includes('優先實現')) recommendations.push('優先實現核心功能');
        if (content.includes('性能優化')) recommendations.push('考慮性能優化');
        if (content.includes('用戶體驗')) recommendations.push('加強用戶體驗設計');
    ...
    private extractSuggestions(content: string): string[] {
        // 簡單的建議提取邏輯
        const suggestions = [];
        if (content.includes('錯誤處理')) suggestions.push('增加錯誤處理');
        if (content.includes('內存使用')) suggestions.push('優化內存使用');
        if (content.includes('單元測試')) suggestions.push('添加單元測試');
        return suggestions;
    }
    ...
    private calculateConfidence(claudeResponse: any, request: ClaudeRequest): number {
        // 基於響應質量計算信心度
