...
class ChatProvider {
    constructor(_extensionUri, _mcpServerManager, authService) {
    ...
    resolveWebviewView(webviewView, context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(async (message) => {
            try {
                ...
                this._sendErrorToWebview(error);
            }
        }, undefined, []);
    }
    refresh() {
    ...
    async _handleChatMessage(text) {
        ...
        try {
            ...
            // 發送到 MCP 服務
            const response = await axios_1.default.post(`${endpoint}/api/process`, requestData, {
                headers: {
    ...
    async _handlePowerAutomationMessage(message) {
        if (!this._authService?.isAuthenticated()) {
