...
LiveKit、Step-Audio 和 Stagewise 三个框架的协同工作将创造一个前所未有的多模态开发体验。我们设计了一个统一的协作架构，充分发挥每个框架的独特优势。

**多模态协作架构**：

底层是 LiveKit 提供的实时语音传输基础设施。所有的语音数据流都通过 LiveKit 的低延迟网络进行传输，为语音交互提供稳定的技术基础。

中间层是 Step-Audio 提供的智能语音处理能力和 Stagewise 提供的可视化交互能力。这两个框架并行工作，Step-Audio 处理语音输入输出，Stagewise 处理可视化指令和上下文传递。

顶层是 AG-UI 协议提供的统一集成接口。所有框架的输出都通过 AG-UI 事件的形式进行标准化处理，实现多模态输入的融合和协调。

**多模态工作流程设计**：

用户可以通过多种方式与系统交互：纯语音模式下，用户通过麦克风发出语音指令，LiveKit 捕获语音数据，Step-Audio 进行语音识别和意图理解，结果通过 AG-UI 协议传递给系统组件。

纯可视化模式下，用户在浏览器中直接指向 UI 元素并输入文本指令，Stagewise 收集上下文信息和用户指令，通过 AG-UI 协议传递给 AI 代理进行处理。

混合模式下，用户可以结合语音和可视化交互，例如通过语音描述总体需求，然后通过 Stagewise 精确指定修改位置。系统会智能地融合多种输入模式，提供最准确的理解和执行。

这种多模态设计不仅提升了交互的灵活性，还显著提高了开发效率和用户体验。
